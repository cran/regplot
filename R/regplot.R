...
#' @param observation  To superimpose an observation, shown in red.  
...
## so cannot be distinguished at this point. 
   lme4     <- glmer | lmer 
   polr     <- (rtype=="polr") 
  if(!( polr | glm | lm | cox | survreg | glm.nb | lme4 )){
...
##  ellipsis ...  additional allowed parameters 
  ...
  # fefault colors
  dencol <- "#EEEEEE"  ## color of density plot fill
  boxcol <- "#B3EADF"  ## color of  factor boxes plot fill
  obscol <- "#FF0000"  ## color of observation points (red) 
  spkcol <- "#669999"  ## color of spike plots
...
## are there NAs in the obsevation? 
...
## must extract coefficients from summary(reg)  
  ...
  if(length(NAcoef)>0){
   message("Model ",deparse(substitute(reg))," has NA beta-coefficients for: ",
   paste(names(reg$coefficients)[NAcoef], collapse=", ") )
   
    ##reg$coefficients <- reg$coefficients[notNA]
    
    message("model has NA beta-coefficents")
   return( message("Rebuild or reformulate model with no NAs"))

  ##  return("Stopping")
    ## NTM: have tried letting this continue with NAs removed from 
    ## reg$coefficients. It half-works. But may crap out. Best to
    ##  put in a stop for NA coefficients? The model v likely to be
    ## ill conditioned.  But could return to it? 
  }
...
## need to bind in  offset data into x, and augment coefficients and varaible_names
 ...
 ##============================================================
 ##   if(points & (lme4 ) ){
 ##     message("Note: \"points=TRUE\" not supported for lme4 models")
## why? Because points associated with  random component not evaluated
 ##    so what:??  Allow.
  ##    points <- FALSE
##     pointscale <- FALSE
 ##     }
 ...
 ## get raw data with this command 
  ...
  {message("Data not data.frame but is: ",
                  paste( class(rawdata), ". Trying coercion!" ))
    ## try coecion
...
##==================================================================
 ## sort out observation value depending how specified
  if(person){
...
## fix up if npars=1, when x is a vector, coerce to matrix
 ...
 ## in coefficient list
    ...
    nointercept <- TRUE
## switch off the "nointercept" indiactor, which is only
...
## alt way to get DF?
## which of ivar are factors?
     arefact <- which(dataclass[ivars]=="factor")
 ## what are levels of these?
     ...
     DF[k] <- dfs
     ##browser()
##  but this fails  when model is pure interaction with no main effects
     }
...
if(sum(DF) != npars ){
##browser()
return( message("Internal checksum fail.  DF",paste0("(",sum(DF),")"),
...
## eg  log(X1 + X2)
  ...
  if(length(test)>0){
    ...
    ## actaull will pass this test but  fails in look_up when clicking
...
##browser()
    ...
    if(length(test2) > 0){
     return(message("variable for ", raw_variable_names[test]," expected but is not in data. Need to create") )
      }
   #  
  }
...
if(FIRSTRUN){BXrawX <- list()
 ...
 gX <- getX(v)
  ...
  ## except for logicals () use actaul variable name
...
#}
    ...
    ##  get confoidence intervals for  "likeafoctor" variable e.g  rcs() or bs()
    ...
    ## lower(ghost=2) and upper (gogst=3) values of beta. I cannot see how to update
...
##  
     ...
     ## abbreviate category labels?  No tuncates " | " to "|" 
   # if(max(beta_values)-min(beta_values ) < (M-L)*0.1){
    ##cats <- abbreviate(cats, minlength=7,use.classes=TRUE) 
    #  } 
...
##  bar essentailly same as spike but with a box.  
...
## NB NTM There is a problem here, identified with interaction
   ...
   plmi <- rep(c(-1,1), times=lb)
    ...
    if(length(plmi) != length(dy)){
     message("checksum error for plmi and dy. Contact Roger")
    }
...
## dont drw intreval for  baseline ??
  ...
  if(isint){
   ...
   ## prefer to show  *  instead of :  for interactions? 
   v <- gsub(":", " * ", v, fixed = TRUE)
...
##========================================================  
  ...
  ##-===============================================================
  ## start overlay in red (obscol) observation dots, text  and lines   
  #=================================================================
     if(person){
...
## output RED value
  ...
  text(totSpos,yposx + 0.1*strata_gap, paste(signif(pred_mean,3)),pos=2,
...
## output RED value
  text(totSpos,yposx+0.1*strata_gap, tS,pos=2,cex=cexscales,font=3,col=obscol  ) 
